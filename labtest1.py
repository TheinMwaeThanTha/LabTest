...
def calc_total_expenses(csv_list):
    ...
    for n in csv_list:
        total_expenses = total_expenses + n
...
def get_items_by_price_range(csv_list, str_price_lower_limit, str_price_upper_limit):
    ...
    for n in csv_list:
        if (float(str_price_lower_limit) <= float(n["price"]) <= float(str_price_upper_limit)):
            result.append(n)
...
def display_main_menu():
    ...
    print("4 - Display all item in alphabetical order")
...
def display_results(result_list):
    for item in result_list:
        print(item["date"] + "\t\t" + item["item"] + "\t\t" + item["price"])
